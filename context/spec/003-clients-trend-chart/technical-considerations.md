...
- **The focus ring must be painted inset.** The focus target fills the card and `Card` has `overflow: clip`, so the project's `outline-offset: 2px` is entirely clipped and invisible even while `:focus-visible` matches. Use `outline-offset: -2px`, or an inset `box-shadow` built from `--focus-ring-width` / `--focus-ring-color` as the tree grid already does.
...
The widget root owns a definite height (`--card-chart-min-h`) as a flex column; the plot box is `flex: 1 1 auto; min-height: 0`; the HTML legend is `flex: none` beneath it; `<ResponsiveContainer width="100%" height="100%">` goes inside the plot box.
...
- **Tint:** `<Tooltip cursor={{ fill: 'rgba(20, 20, 19, 0.04)' }} />` — verified with the literal; it spans one whole band. The `var(--color-row-hover)` form is valid as an SVG presentation attribute but was *not* measured, so the lane must confirm it or use the literal via a CSS module.
- **Outside tap:** a document `pointerdown` listener attached **only while open**, closing when the target is outside the widget root. Without it, tapping the table card left the panel open — measured. (The stock tooltip only appeared to dismiss because tapping a bar had focused a Recharts `<g>`, and the next tap blurred it; once focus stealing is prevented, that accident is gone.)
...
| R-4 | **Pointer resting on a month while Tab arrives:** the panel shows the hovered month while the live region says February — Recharts' internal hover index beats `defaultIndex`. | Render the panel's text from the widget's own `index`, never from Recharts' payload (§2.1). The tint stays on the hovered month until the pointer moves; accepted as low severity. |
