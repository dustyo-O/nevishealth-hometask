...
| D-17a | **`--table-name-col-w` narrows to 160 px below ~600 px viewport width** (owner's decision 2026-09-22, after the acceptance suite caught it). | At 375 the card is 343 wide; the design's 264 px name column leaves 79 px, less than one 88 px month, so a figure is clipped at every scroll position. 160 px leaves 183 px — two whole months — while the row stays identifiable. The deepest level then has ~52 px of text room, so names can truncate there: FR5's ellipsis, `title` and focus reveal already handle it, and FR6's criterion was amended to match. | Keeping 264 everywhere (a permanently clipped column); shortening the month headings (loses the year on phones). |
