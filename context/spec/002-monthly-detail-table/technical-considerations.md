...
| D-14 | **The root ESLint config gains `treegrid` to `jsx-a11y`'s allowlist for `table`.** | Verified against the repo's own flat config: `<table role="treegrid">` raises `jsx-a11y/no-noninteractive-element-to-interactive-role`, whose allowlist is `table: ['grid']` — the plugin's list is simply incomplete; WAI-ARIA permits `treegrid` on `table`. The root config is **not** `react-frontend`'s to edit, so this is a **lead/scaffold task, done before the lane starts**, or the web gate is red on its first commit. | A scoped `eslint-disable` in the widget (hides the same issue from every future table). |
...
| D-16 | **Assumption:** FR2's "slide in / slide out" is satisfied by auto-animate's default scale-and-fade (the rows *below* do slide, via its FLIP translate). If literal sliding is wanted, a plugin returning `translateY` keyframes is ~10 lines. **Decide before the lane starts** — the spec's wording and the library's default disagree, and a reviewer will notice. | | |
| D-17 | **Assumption:** `--table-month-col-min-w: 72px` — the only geometry not in Figma. It sets the scroll width at 375 (264 + 12 × 72 = 1128, measured 1133 with borders). | | |
...
Geometry, all from Figma `1:2901`: `--table-row-h: 56px`, `--table-header-h: 56px`, `--table-name-col-w: 264px`, `--table-indent-step: 28px`, `--table-cell-pad-inline-start: 16px`, `--table-cell-pad-inline-end: 24px`, `--table-chevron-size: 16px`, `--table-chevron-gap: 8px`, `--table-avatar-size: 20px`, plus `--table-month-col-min-w: 72px` _(proposal, D-17)_.
...
- **Grill R3 (animation) — closed** by D-8; the entry-only fallback stays documented. Cheap early check if the reconciler and the library ever fight: an e2e that toggles Branch 1 ten times quickly and asserts the final row count.
...
- **New — leaving rows linger ~250 ms** with stale `aria-posinset` and `tabindex="-1"`. Harmless in use (`pointer-events: none`), but every test must `waitFor` their removal rather than count rows immediately after a collapse.
...
| `shared/ui/tree-grid/ui/tree-grid.test.tsx` | Tab in once / Tab out (FR3-AC1); exactly one `tabindex="0"`; the three level attributes; `aria-expanded` absent on leaves (FR4-AC5); Enter/Space (FR3-AC12); focus recovery on collapse (FR2-AC6/AC7); `jest-axe` |
...
| `widgets/clients-table/ui/clients-table.test.tsx` | figures verbatim and Company = 250/350 rather than a sum (FR1-AC2); the blank first header announced as "Name" (FR4-AC3); `headers` on every figure (FR4-AC2 at DOM level); clicking a figure changes nothing (FR2-AC5); mouse expand/collapse (FR2-AC1/AC2/AC4); `jest-axe` on an expanded table |
