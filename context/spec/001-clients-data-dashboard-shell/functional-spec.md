...
- **Acceptance Criteria:**
  ...
  - [x] When the service starts with the delivered dataset, then it prints one warning for each figure that does not equal the sum of the items beneath it — seven, with the payload as supplied (amended by spec 004; it printed none while we served a completed dataset). _(Verified 2026-09-22: verified: production smoke on the built app — "0 discrepancies"; the running dev API logs `Checked 44 nodes: 0 discrepancies`)_
...
- [2026-09-23] — spec 004, which restores the payload the brief supplies — **the data decision this spec recorded is reversed, and the consistency check now has something to report.** This spec served a dataset we had completed: advisers invented for two branches with none, channels invented for four advisers without them, and three figures corrected so every parent equalled the sum of its children. The brief says the nesting is deliberately not uniform and that the UI has to handle it, so completing the data inverted the requirement. The payload is now served exactly as supplied, gaps and all, and the seven places where a parent disagrees with its children are reported on start-up and served anyway — which is what this spec always said the check should do. The criterion that the delivered dataset prints no warnings is amended above.
