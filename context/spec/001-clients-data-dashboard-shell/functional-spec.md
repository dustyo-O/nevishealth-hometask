...
- **Acceptance Criteria:**
  ...
  - [ ] Given a test dataset in which one item carries two kinds of list beneath it (e.g. a branch with both advisers and channels), when the user opens the page, then they see the error panel with the detail "Unexpected data shape".
...
- **Acceptance Criteria:**
  ...
  - [ ] When a reviewer opens `http://localhost:5173/?delay=3000&fail=1` in development, then the placeholder cards stay for at least 6 seconds (two delayed attempts) and the error panel then appears within 9 seconds of opening.
