...
- **Non-uniform hierarchy handled gracefully** — the served data is the supplied payload, in which Branch 2 and Branch 3 have no advisers and only Anna Blackwood has channels. A node without children is rendered as a leaf: it shows its numbers and offers nothing to expand. This is the brief's own test of the UI, not an edge case.
...
- The payload (`context/inbox/data.json` — the supplied data, **exactly as the brief gives it**, gaps and disagreements included) served from a small REST API; loading and error states handled in the UI.
...
- **What the chart stacks by:** acquisition channel, as the design's legend says and as the design frame confirms — its bars reach the Company row's own figures. **Amended 2026-09-23 (spec 004):** we first read the supplied data's gaps as defects and generated channels for every adviser and advisers for Branch 2 and 3, so the split could be summed from the tree. That inverted the brief, which says the nesting is deliberately not uniform and the UI has to handle it. The payload is now served exactly as supplied, and the company's existing clients are derived — its own figure less the newly acquired, which are the only channel figures the data records. _(README: what we assumed, why it was wrong, and what we changed.)_
- **Stored totals vs. sum of children:** the supplied data disagrees with itself in seven places — Company May (301 against 279), Branch 1 Aug (214 against 216) and five months of Anna Blackwood. **Amended 2026-09-23 (spec 004):** we first corrected them so every parent equalled its children; they are now served untouched. The figures shown are always the stored ones, never recalculated, and the service reports all seven on start-up and serves the data anyway. _(README: list the seven; note that in production the API, not the UI, should own this invariant.)_
