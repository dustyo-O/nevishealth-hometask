...
- **UI state:** React state for anything one widget owns alone. **One shared store — Zustand, in `features/expand-row/`** (amended 2026-09-23, spec 004): which rows are open stopped being the table's private business when the chart began dividing itself by exactly the rows the table displays. Two sibling widgets need the same value, and threading it through the page would make the page the coupling point between them; §6 already reserved this slice for the day it earned its keep. Zustand over the alternatives for size (~1.2 kB gzip) and for needing no provider — and deliberately *not* Redux Toolkit, even though Recharts already ships it as a dependency, because pinning our version to a chart library's transitive dependency is a trap.
...
apps/web/src/
  ...
  features/
    expand-row/        the Zustand store of open row ids — the one thing the table and the chart share
