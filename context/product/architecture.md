...
- **Charting:** Recharts 3 — `BarChart` with three stacked `Bar`s (`stackId`) and `Tooltip`. Data mapping is a pure function in the `entities` layer (`toMonthlySeries(tree)`), unit-tested independently of Recharts. A visually-hidden summary table backs the chart for screen readers.
