...
- **API (Vitest + supertest — Nest 12 is ESM-only and Jest cannot load it on Node 22, spec 001 D-1):** `GET /api/clients` returns the envelope with 12 values per node; consistency check passes on the shipped data; dev switches gated off in production.
